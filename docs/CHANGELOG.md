...
### Docs / Public page tone cleanup v4.5.25

1. 清理 README、文档首页和用户教程中的开发过程说明、旧 UI 对比、导航验收口径和维护规则，让公开页面只描述当前产品能力和使用方式。
2. 从 MkDocs 中英文导航和站点构建中移除文档评审迭代页；评审记录保留为仓库历史材料，不再作为网页内容发布。
3. 将提示词构成页改为用户可读说明，删除源码位置、维护同步规则和测试约束说明；单元测试改为校验页面中的中英文运行时 prompt 结构。
4. 从公开导航中移除 Agent Onboarding 和 Legacy Research Pipeline，避免把 agent 接手材料和历史迁移说明作为普通读者入口。
