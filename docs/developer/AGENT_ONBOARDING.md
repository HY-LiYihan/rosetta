...
1. [Home.py](../../app/ui/pages/Home.py)：项目总览，显示项目状态和入口。
2. [Concept_Lab.py](../../app/ui/pages/Concept_Lab.py)：定义与规范，维护概念阐释、15 条金样例、概念自举和 prompt training。
3. [Batch_Run.py](../../app/ui/pages/Batch_Run.py)：批量标注，导入 TXT / JSONL / CSV，提交本地任务。
4. [Review_Queue.py](../../app/ui/pages/Review_Queue.py)：审核与修正，逐条处理低置信、低自洽或抽检样本。
5. [Export_View.py](../../app/ui/pages/Export_View.py)：结果与导出，输出 JSONL、报告和统计。
...
| `app/workflows/review` | 审核与修正、候选选择、hard example、promote-to-gold |
