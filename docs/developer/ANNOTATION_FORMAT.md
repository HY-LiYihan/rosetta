...
从 `v4.5.9` 开始，概念验证、候选回测、单条标注和批量标注应共用同一个运行时 prompt 框架；从 `v4.5.20` 开始，`build_runtime_annotation_prompt()` 中的段落标题和 system prompt 由程序常量维护，并在 [提示词构成](../user/PROMPT_COMPOSITION.md) 中提供 `zh-CN / en-US` 对照说明：
...
{ReferenceExamples retrieved by top-k embedding, or 无}
...
通用格式示例只能解释返回结构，不能使用当前 gold、相似样例、失败样例、待标注文本或任何任务答案片段。相似参考样例有专门槽位，只有 top-k 检索或批量上下文显式传入时才填充；普通概念样例仍可用于标签推断，但不会自动作为 few-shot 答案进入标注 prompt。批量标注中的相似样例、边界远例和失败记忆只能作为概念边界上下文，不应替代或污染 `标注格式` 段落。

从 `v4.5.10` 开始，所有标注型调用还应共用同一个 system prompt。默认 `zh-CN` 内容为：

```text
你是严谨的标注助手，只输出 JSON。
```

程序也提供同构的 `en-US` system prompt：

```text
You are a rigorous annotation assistant. Output JSON only.
```

这条 system prompt 同时用于概念验证、候选回测、单条标注和批量标注。它只定义模型身份和输出纪律；具体概念、冻结标注格式、待标注文本和任务强调全部放在 user prompt 中。这样可以避免“概念校验助手”“批量标注助手”等不同身份造成额外变量，使三方法优化实验和批量标注使用同一标注助手条件。主流程当前默认使用 `zh-CN`，调用方显式传入 `prompt_language="en-US"` 时可生成英文控制模板；用户输入和任务文本不会自动翻译。
...
2. 定义优化时，模型输出按 JSON+markup 协议解析，再和 gold spans 计算 loss。
