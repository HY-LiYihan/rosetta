...
1. v4.5.16 已将 debug 模式 `/debug` 调试追踪页改成隐藏导航的实时日志页，默认读取最新 debug session 并每 2 秒刷新。
2. v4.5.15 已新增 debug 模式 `/debug` 调试追踪页，完整展示 OpenAI-compatible provider 的 LLM prompt / response 子对话；该能力只用于本机排障，日志包含敏感内容。
3. v4.5.12 已将三种 prompt optimizer canonical 化、默认并发上限提升为 50，并新增本地轻量 embedding 检索 `rosetta-local-hash-384`。
4. v4.5.1 已实现三方法真实对比实验：每个方法连续 5 轮 loss 无下降才停止，CLI 输出 Markdown 报告、完整 JSON trace 和提示词演化 JSONL。
5. v4.5.0 已实现 LLM service runtime 最小闭环：DeepSeek 默认 `deepseek-v4-pro`，provider 并发上限默认 50，提示词训练记录调用、token、耗时、progress event 和去语料化修复统计。
