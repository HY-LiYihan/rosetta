...
2. [Architecture](./ARCHITECTURE.md)：确认运行结构、目录分层和数据流。
3. [Workflow](./WORKFLOW.md)：确认开发、验证、提交和文档评审规则。
4. [Scripts](./SCRIPTS.md)：确认 CLI、部署和 legacy 入口。
5. [Deployment](./DEPLOYMENT.md)：确认 Docker、runtime 目录和运维方式。
...
1. v4.2.3 文档主线已整理为 user / developer / research claims 三条入口，并加入三角色评审记录。
2. v4.2.2 概念自举使用 loss-guided candidate search，只接受 gold loss 下降的干净概念版本。
3. v4.2.1 修正概念修订边界：`ConceptVersion.description` 只保存干净提示词，失败摘要和原始响应进入 `metadata`。
4. v4.2.0 默认主线是 concept bootstrap loop：15 条金样例校准、概念版本、失败摘要、增强上下文标注和主动审核反馈。
5. 工作台收敛为轻量状态入口，概念实验室内置“硬科学科普术语标注”填表示例。
6. 本地批量任务队列使用 SQLite 存储 `jobs / job_items / job_events`。
7. 概念阐释与金样例模型使用 `concept_guidelines / gold_example_sets / concept_versions` 存储。
8. `Annotate` 单条旧流程保留为兼容页面；新主流程优先走 `app.workflows.annotation.batch`。
9. Bootstrap 和 Corpus 通过 `app.workflows.*` 包装 legacy 实现。
10. 统一 CLI 为 [scripts/tool/rosetta_tool.py](../../scripts/tool/rosetta_tool.py)。
11. `app/research` 与 `app/corpusgen` 不再作为新功能边界，只做兼容层。
