...
当前文档是愿景与设计契约，不表示代码已经完整实现。现有代码中，批量标注已经有本地线程池并发；概念验证和概念自举仍是串行。后续 v4.4-v4.6 需要按本文档逐步收敛。
...
```text
global_default_max_concurrency = 10
```

含义：

1. 默认情况下，任何单个 provider 的并发上限不超过 10。
2. UI 可以允许用户选择并发，但最大值应受 provider profile 与全局上限共同限制。
3. 如果某个平台明确限流更低，则使用更低值。
4. 如果本地 mock 或未来本地模型允许更高并发，也应显式声明，不应偷用真实 API 默认值。
...
3. 调度器按 provider profile 并发执行，默认并发 5，上限 10。
...
3. 调度器按 provider profile 控制并发，真实 API 默认并发 10。
...
1. 新增 provider 级 semaphore。
2. 概念验证、自举循环、批量标注统一走调度器。
3. 默认并发上限改为 10。
...
1. 概念验证 15 条金样例不再串行，默认并发 5，上限 10。
2. 批量标注真实 API 默认并发 10，并受 provider profile 限制。
