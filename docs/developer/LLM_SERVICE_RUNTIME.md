...
```text
global_default_max_concurrency = 20
```

含义：

1. 默认情况下，任何单个 provider 的并发上限不超过 20。
2. UI 可以允许用户选择并发，但最大值应受 provider profile 与全局上限共同限制。
3. 如果某个平台明确限流更低，则使用更低值。
4. 这里的 `20` 是 Rosetta 默认上限，不是绕过平台限流；rate limit 由 provider profile、重试和退避策略处理。
...
| --- | --- | --- |
| 概念验证 15 条金样例 | 20 | 20 |
| Prompt training 三方法验证 | 20 | 20 |
| 概念自举候选评估 | 20 | 20 |
| 批量标注真实 API | 20 | 20 |
| 批量标注本地 mock | 20 | 20 |
| LLM-as-a-judge | 20 | 20 |
| 语料生成 | 20 | 20 |
...
3. 调度器按 provider profile 并发执行，默认并发 20，上限 20。
...
2. 提示词优化训练的 gold validation、候选回测和方法对比已走 runtime-backed predictor；批量标注默认并发已调到 20，后续继续把 provider 调用完全迁入 runtime。
3. 默认并发上限改为 20。
...
1. 概念验证 15 条金样例不再串行，默认并发 20，上限 20。
2. 批量标注真实 API 默认并发 20，并受 provider profile 限制。
