...
下一阶段要把当前 `loss-guided candidate search` 升级为显式 Prompt-as-Parameter 优化。概念阐释不再作为整段字符串直接交给 LLM 改写，而是先切分为可训练文本参数，再估算 Text Gradient，最后由优化器生成候选。
...
当前状态边界：`v4.2.2` 已实现 loss-guided candidate search；完整 `PromptSegmenter / TextGradientEstimator / PromptOptimizer` 尚未实现。`v4.2.4` 只把这套方法写入文档，作为后续 `v4.3` 开发依据。
