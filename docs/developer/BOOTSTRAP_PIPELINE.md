...
这条流水线只放在 `app/research/*` 和 `scripts/research/*`：

```text
app/research/
  bootstrap_contracts.py     # 数据契约
  bootstrap_io.py            # JSONL 读写与校验
  consistency.py             # k-run 自洽性评分
  human_review.py            # 专家复核队列与多候选选择题
  contrastive_retrieval.py   # 相似样例 + 边界远例检索
  label_statistics.py        # entity/context/other token 统计
  reflection.py              # 反思计划，不直接调用 UI
  bootstrap_runner.py        # 端到端编排

scripts/research/
  run_bootstrap.py           # CLI 入口
```
...
3. `app/research/indexing.py`：CPU embedding index。
