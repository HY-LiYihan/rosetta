...
1. 第一层（优先）：`.streamlit/config.toml` 主题配置。
2. 第二层（兜底）：`streamlit_app.py` 的最小 CSS 覆盖。

适用边界：

1. 可放 TOML：主题色、背景色、文本色、链接色、字体、圆角、边框、sidebar 主题色。
2. 需保留 CSS：`data-testid` 选择器定向覆盖、`:hover` 交互态、移动端媒体查询、导航密度微调。

维护要求：

1. 新样式需求优先评估是否可用 TOML 实现。
2. 只有 TOML 不支持的能力才允许加 CSS。
3. 避免大面积覆盖 Streamlit 内部 DOM，降低版本升级破坏风险。
