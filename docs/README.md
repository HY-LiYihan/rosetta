...
1. v4.5.4 已实现概念验证并发与页面进度条：真实 LLM 模式默认并发上限 20，并显示完成数、运行中数量、ETA、token 和耗时。
2. v4.5.3 已实现主运行库重启自动清洁：默认只保留官方“专业命名实体标注”项目、一个概念、15 条金样例和一个初始概念版本。
3. v4.5.2 已实现提示词优化训练后台运行、SQLite 实时进度事件、Concept Lab 轮询进度卡片、折叠日志和 `run_events.jsonl`。
4. v4.5.1 已实现三方法真实对比实验：每个方法连续 5 轮 loss 无下降才停止，默认最多 30 轮；CLI 输出 `comparison_report.md / comparison_result.json / prompt_evolution.jsonl`，并按历史最优接受版本汇总最佳提示词。
5. v4.5.0 已接入统一 LLM service runtime：DeepSeek 默认 `deepseek-v4-pro`，默认 provider 并发上限 `20`，提示词训练和批量标注记录调用、token、耗时和修复统计。
6. v4.4.1 已实现提示词优化训练防背答案检查：训练反馈可看批改对照，learned operational prompt 必须无语料和答案片段泄露。
7. v4.4.0 已实现提示词优化训练：同一批 15 条金样例比较 `llm_optimize_only / llm_reflection / text_gradient_adamw`。
8. v4.3.1 文档化 LLM service runtime 愿景：每次大模型调用都应作为服务调用被限流、排队、追踪、计费和可视化，默认并发上限后续收敛为 20。
9. v4.3.0 已实现 Prompt-as-Parameter 最小内核：prompt 分段、Mask 文本梯度、LLM-AdamW trace、长度惩罚和 loss 验证。
10. v4.2.4 将 Prompt-as-Parameter、Text Gradient 和 `LLM-AdamW` 写成核心方法框架。
11. v4.2.3 将文档重构为 user / developer / research claims 三条入口，并记录 6 轮三角色文档评审。
12. v4.2.2 将概念自举升级为 loss-guided candidate search：每轮比较当前概念和候选概念的 gold loss，只接受变好的版本。
13. v4.2.1 修正概念自举修订：最终提示词只保存干净概念阐释，失败摘要、样例编号和模型原始响应只进入日志与 metadata。
14. Streamlit 仍是唯一正式 UI。
15. 工作台收敛为轻量状态入口，概念实验室启动即显示“专业命名实体标注”官方样例。
16. 概念实验室负责概念自举和提示词优化训练，批量标注负责 TXT/JSONL/CSV 导入和本地任务队列。
17. 审核队列按置信度、抽检和错误类型逐条展示待审核样本，并沉淀 hard examples。
18. Prodigy-compatible JSONL 不推翻，只增强 project/run/session/job 层。
19. 旧 `research/corpusgen` 暂不删除，作为 compatibility wrapper 的实现来源。
