...
1. v4.2.0 将主工作流升级为 concept bootstrap loop：15 条金样例校准、概念版本、失败摘要、增强上下文标注和主动审核反馈。
2. v4.1.2 为长耗时按钮增加运行中禁用状态，避免重复触发验证、批量提交和审核保存。
3. Streamlit 仍是唯一正式 UI。
4. 工作台收敛为轻量状态入口，概念实验室内置“硬科学科普术语标注”可填表示例。
5. 概念实验室负责概念自举，批量标注负责 TXT/JSONL/CSV 导入和本地任务队列。
6. 审核队列按置信度、抽检和错误类型逐条展示待审核样本，并沉淀 hard examples。
7. Prodigy-compatible JSONL 不推翻，只增强 project/run/session/job 层。
8. 旧 `research/corpusgen` 暂不删除，作为 compatibility wrapper 的实现来源。
