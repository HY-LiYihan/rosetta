...
1. v4.1.0 将默认 UI 收敛为 5 个中文主页面。
2. Streamlit 仍是唯一正式 UI。
3. 概念实验室负责概念阐释与金样例，批量标注负责 TXT/JSONL/CSV 导入和本地任务队列。
4. 审核队列按置信度和抽检策略逐条展示待审核样本。
5. Prodigy-compatible JSONL 不推翻，只增强 project/run/session/job 层。
6. 旧 `research/corpusgen` 暂不删除，作为 compatibility wrapper 的实现来源。
