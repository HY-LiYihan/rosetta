...
1. v4.1.2 为长耗时按钮增加运行中禁用状态，避免重复触发验证、批量提交和审核保存。
2. v4.1.1 将语言切换移动到侧栏顶部，并让 5 个主页面正文同步中英文切换。
3. Streamlit 仍是唯一正式 UI。
4. 工作台收敛为轻量状态入口，概念实验室内置“硬科学科普术语标注”可填表示例。
5. 概念实验室负责概念阐释与金样例，批量标注负责 TXT/JSONL/CSV 导入和本地任务队列。
6. 审核队列按置信度和抽检策略逐条展示待审核样本。
