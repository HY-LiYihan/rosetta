...
公开教程按中文界面维护，页面名称、按钮和示例都以中文为准。文档中的英文主要保留在文件格式、命令、模型名、字段名和代码标识符中，例如 `JSONL`、`report.md`、`deepseek-v4-pro` 和 `[span]{Term}`。用户输入、数据库内容、任务文本、模型输出、标签值、调试日志和导出文件名不会被自动翻译。
