...
页面顶部提供“提示词验证 / 提示词优化”两个入口。先验证当前定义，再根据验证结果决定是否优化。
...
边界、排除和特殊情况都写进“当前概念阐释”正文里；定义优化也只改这一段可优化定义。标签会从金样例里的 span label 自动推断；如果推断不到，系统默认使用 `Term`。标注输出协议提供两个选项：
...
也就是说，提示词优化训练不会去“训练”标签、JSON schema 或输出格式。它只尝试把概念规则写得更清楚；模型每次实际标注时，Rosetta 会加入固定输出格式，并检查返回结果是否合格。

注意这里有两层格式：**模型运行时格式**可以是简单的 `[span]{Term}`，便于 LLM 稳定输出；**最终存储格式**仍是 Rosetta 统一的结构化标注格式，保留 spans、relations、meta 和导出所需字段。

模型真实标注时，Rosetta 会把 prompt 固定拼成六段。中英文模板的完整对照见 [提示词构成](./PROMPT_COMPOSITION.md)：
...
在 gold 验证中，top-k 参考样例只用于观察“带检索上下文时是否更稳定”，不能当作 held-out 泛化评测。严格实验应排除当前待测 gold 自身，并在报告中记录参考样例来源；无样例标注验证仍是判断当前定义是否独立可执行的主诊断。
...
提示词优化训练会区分“批改参考”“候选定义”和“最终定义”：

1. 批改参考可以包含原文、标准答案和模型自己的错误回答，用来帮助判断概念边界哪里不稳定。
2. 最终定义不能复制原文、具体答案片段、gold span 或模型 span。它只能把错误抽象成边界规则、排除规则和输出要求。
3. Rosetta 会拦截这类复制答案的候选，并在报告中只展示安全摘要、hash 和数量。
4. 日志区可以按事件类型和阶段筛选，也可以下载 `run_events.jsonl`。这个文件记录训练过程中的阶段、候选状态、模型调用、token、ETA 和错误摘要。
5. 这一轮只测试 15 条 gold，因此结果只能说明当前 gold 集合内的训练表现。如果要写论文或证明泛化，后续还需要 held-out 数据。

系统也会区分“格式错误”和“概念错误”：先检查 JSON 与 markup 格式，再统计漏标、多标和边界错误。这样你能知道问题到底是模型没有按格式输出，还是概念规则本身不够清楚。
...
“批量标注”负责把原始语料变成任务队列。批量标注会自动构建标注上下文：
...
1. 每条采样次数：1、3 或 5。
2. 并发数：默认 50。真实 API 小样本测试建议先手动降到 1-5。
3. 人工审核阈值：默认 0.75。
4. 高置信抽检比例：默认 5%。
...
| 调用大模型 | 正式标注或真实实验 | 是一次真实模型运行；仍需记录 provider、model、并发、token 和运行报告，并保留人工审核 |
...
应用侧栏顶部提供 `中文 / English` 两个切换按钮。当前语言按钮会高亮，点击另一个按钮会切换主导航和主要固定界面文案。

公开教程按中文界面维护，页面名称、按钮和示例都以中文为准。文档中的英文主要保留在文件格式、命令、模型名、字段名和代码标识符中，例如 `JSONL`、`report.md`、`deepseek-v4-pro` 和 `[span]{Term}`。用户输入、数据库内容、任务文本、模型输出、标签值、调试日志和导出文件名不会被自动翻译。模型运行时提示词的中英文组成见 [提示词构成](./PROMPT_COMPOSITION.md)；界面语言按钮不会自动翻译你的概念阐释或待标注文本。

## 8. 完整使用案例：专业命名实体标注

这个案例是 Rosetta 的官方样例。程序启动后会准备这个项目，并内置 15 条金样例和基础提示词。
