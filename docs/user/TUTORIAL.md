...
这个页面先做四件事：选择或创建标注项目；填写概念名称、概念定义和边界说明；选择冻结的标注输出协议；维护 15 条金样例作为标准答案，并运行验证、自举校准或提示词优化训练。
...
2. `失败反思` 和 `文本梯度 AdamW` 可以利用批改参考或文本梯度，但生成的候选仍只能改概念定义、边界规则和排除规则。
3. 输出协议被冻结为 JSON+markup：`text / annotation / explanation` 三个核心字段，`annotation` 使用 `[span]{Term}`。
4. 如果模型返回了非法 JSON、漏字段、字段外多余解释、span 找不到或 label 不合法，系统应先尝试最多 2 次格式修复。
5. 格式修复只修 JSON 和 markup，不改概念语义；修复成功后才计算漏标、多标和边界错误。
6. 后续实验报告会把“格式错误率”“格式修复成功率”和“语义 loss”分开显示，这样你能知道问题到底是模型不会按格式输出，还是概念规则本身不够好。
