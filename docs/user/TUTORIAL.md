...
1. 候选搜索优化只让大模型优化概念语义，不给它看原文、标准答案、错误回答、loss、失败摘要或文本梯度。
2. 批判器 AdamW 优化会先给当前可优化提示词，再把每个失败 detail 按“原文 -> 标准答案 annotation -> 模型回答 JSON -> 错误摘要”放在同一个小块里，方便模型看懂同一句哪里错了。
3. 批判器 AdamW 优化和遮挡梯度优化可以利用批改参考或文本梯度，但生成的候选仍只能改概念定义、边界规则和排除规则。
