...
这个页面先做四件事：选择或创建标注项目；确认概念定义、边界规则和负例规则；维护 15 条金样例作为标准答案；运行验证、自举校准或提示词优化训练，得到后续批量标注可直接使用的稳定规范。
...
从 `v4.5.5` 的文档契约开始，你可以把这里分成两件事理解：

1. 你需要认真写的是“概念语义”：概念定义、边界说明和负例规则。
2. Rosetta 负责固定“输出协议”：标签、JSON 字段、`[span]{Term}` 标注格式、格式校验和格式修复。

也就是说，提示词优化训练不会去“训练”JSON schema 或输出格式。它只尝试把概念规则写得更清楚；模型每次实际标注时，Rosetta 会把冻结的输出协议注入进去，并检查返回结果是否合格。
