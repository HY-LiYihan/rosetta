...
这份教程优先面向第一次接触数据标注工具的用户。你不需要先理解模型训练、PLM 或 agent 架构，只需要按页面顺序完成：定义任务与标注规范、给出 15 条标准答案、让系统校准、批量处理文本、审核不确定样本、导出数据。
...
4. **提示词优化训练**：系统把概念阐释当作可训练参数，在同一批金样例上生成候选、计算 loss、只接受变好版本；达到 15/15 或连续 5 轮无下降时停止。
...
2. 在“提示词验证”中运行格式验证、无样例标注验证或 top-k 参考样例验证，再在“提示词优化”中选择人工优化、无样例自监督优化或类训练优化。
...
“定义与规范”负责产出稳定概念阐释和金样例库。它是 Rosetta 最重要的页面：如果概念没有校准好，后面的批量标注会放大错误。

这个页面先做四件事：选择或创建标注项目；填写概念名称、概念定义和边界说明；选择冻结的标注输出协议；维护 15 条金样例作为标准答案，并运行提示词验证或提示词优化。
...
从 `v4.5.8` 开始，新建临时概念时不再要求你手写标签集合和输出格式。标签会从金样例里的 span label 自动推断；如果推断不到，系统默认使用 `Term`。标注输出协议改为选项：
...
1. **可优化定义**：你需要认真写的是任务定义、概念定义和边界说明。提示词优化训练只会改这一栏。
...
通用格式示例只讲 JSON 和 `[span]{Term}` 或 AnnotationDoc 应该长什么样，不使用你的 gold 句子、gold 答案或当前待标注文本。这样可以让模型知道怎么返回结果，又不会把标准答案混进 operational prompt。
...
top-k 参考检索默认使用 `rosetta-local-hash-384` 本地 embedding，不调用智谱或 DeepSeek embedding API，不消耗 token。真实 LLM 验证默认并发上限为 `50`；页面会显示进度条、运行中数量、已用时、预计剩余时间、调用数、token 和模型耗时。验证时使用的模型身份和批量标注一致，都是“严谨的标注助手，只输出 JSON”。验证结果会分成通过、失败、边界不稳定三类；最终保存的概念阐释只保留可直接用于标注的提示词正文。
...
1. 每条采样次数：1、3 或 5。
2. 并发数：默认 50。这个值是 Rosetta 的 provider 级默认上限；如果平台限流，系统会按 provider profile 和重试策略等待。
3. 人工审核阈值：默认 0.75。
4. 高置信抽检比例：默认 5%。
...
| --- | --- |
| 概念名称 | 专业命名实体 |
| 概念描述或定义 | 标出英文科学与技术文本中具有明确领域含义、可命名且边界清楚的专业实体。 |
...
标签 `Term` 来自官方 gold 标注并作为冻结输出协议注入，不需要在表单里手填。官方样例没有把负例规则作为主输入项暴露；如果后续任务确实需要复杂排除条件，应优先写成边界说明里的“不包括……”规则，或者在下一版高级设置中单独扩展。
