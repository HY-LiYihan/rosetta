...
“验证概念”可以先用本地结构验证跑通流程，也可以选择已配置的大模型对金样例试标。真实 LLM 模式会并发验证 15 条 gold，默认并发上限为 `20`；页面会显示进度条、运行中数量、已用时、预计剩余时间、调用数、token 和模型耗时。验证时使用的模型身份和批量标注一致，都是“严谨的标注助手，只输出 JSON”；区别只在这次 user prompt 里放的是 15 条金样例验证文本。验证结果会分成通过、失败、边界不稳定三类，失败样例会进入修订日志；最终保存的概念阐释只保留可直接用于标注的提示词正文。
...
| 失败反思 | 先给当前提示词，再逐条给失败样例的原文、标准答案 `[span]{Term}`、模型 JSON 回答和错误摘要，让大模型把具体错误抽象成整体概念阐释 |
