...
从 `v4.5.7` 开始，页面会直接把这里分成两栏：

1. **可优化定义**：你需要认真写的是任务定义、概念定义、边界说明和负例规则。提示词优化训练只会改这一栏。
2. **冻结输出协议**：Rosetta 负责固定标签、JSON 字段、`[span]{Term}` 标注格式、格式校验和格式修复。这一栏会被锁定注入给模型，不交给优化器改写。
