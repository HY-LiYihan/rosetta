...
2. 概念实验室提供“开始自举校准”，展示每轮通过数、失败样例、失败摘要和最终概念草案。
...
1. 还没有真正的 embedding-based semantic retrieval；当前第一版使用轻量 lexical similarity。
2. 还没有 LLM-as-a-judge 候选评审。
3. 还没有 token logprob 或 semantic entropy。
4. 还没有完整 ablation runner 和数据集级 F1 对比表。
