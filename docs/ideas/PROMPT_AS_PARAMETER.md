...
适用场景：判断 `ConceptPromptSpec` 里的任务定义、概念定义、边界规则和排除规则是否真的有效。

风险：模块之间有交互，单独贡献不等于联合贡献。

如果要评估输出协议、参考样例或格式修复，它们应单独作为 harness / context ablation，不应混进可优化 prompt 参数消融里。
...
文本梯度估算之后，需要一个 prompt 优化器决定如何更新 `ConceptPromptSpec`。
...
Rosetta 文档默认推荐 `AdamW-like` 文本候选控制器作为 v1 概念优化器叙事：
...
这里的 `LLM-AdamW` 只是一种叙事标签，不应被误读为已经实现了完整数值 AdamW 状态机。
