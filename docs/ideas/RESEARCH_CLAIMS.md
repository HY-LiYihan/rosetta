...
1. **低资源样本效率**：在只有一句话概念描述和 15 条金样例时，检验 Rosetta 是否能比 zero-shot、普通 few-shot ICL、只检索相似样例、以及低预算 PLM fine-tuning 更快达到预定义 F1 或审核成本阈值。
2. **非标准任务适应性**：当任务不是成熟 NER 标签集，而是“可清晰定义但没有现成大数据集”的概念标注时，检验 Rosetta 是否能把概念描述逐步操作化，并把失败模式沉淀为可复用边界规则。
3. **Prompt-as-Parameter 优化**：检验 Rosetta 是否能把自然语言中的可优化定义当作文本参数，在无解析梯度条件下估算 Text Gradient，并用优化器更新概念阐释。
4. **可审计主动学习收益**：检验 Rosetta 是否能把人类专家时间集中到低自洽、低置信、规则失败和 hard examples 上，并证明选择题式审核比从零标注更省人工。
...
3. **Prompt-as-Parameter Optimization**：概念阐释不是静态 prompt；当前可优化参数只包括任务定义、概念定义、边界规则、排除规则和抽象失败模式。标签、JSON schema、markup、parser、format repair、gold 原文和 gold 答案都属于冻结 harness 或训练反馈，不属于最终可优化定义。
