...
它不是普通的“上传文本后调一次大模型”页面，也不是 `research` 与 `corpusgen` 两条科研脚本的集合。Rosetta 围绕真实标注路径组织能力：
...
`v4.4.0` 起，“定义与规范”新增 **提示词优化训练**：系统可以在同一批 15 条金样例上公平比较三种定义优化方法，当前 canonical 名称为 `sgd_candidate_search / critic_adamw_optimizer / mask_guided_optimization`。每种方法独立运行，每轮生成候选提示词、回到金样例上计算 loss，只接受损失下降的干净版本；最终只把胜出的可用提示词写入 `ConceptVersion.description`，方法对比、loss 曲线、候选日志和原始响应进入 metadata / artifact。
...
`v4.5.0` 起，提示词优化训练接入统一大模型服务运行时：默认真实模型路径使用 DeepSeek `deepseek-v4-pro`，平台级共享并发上限提升到 `50`。15 条金样例验证和候选回测会受同一运行时约束；批量标注当前仍使用本地任务队列和页面线程池调用 provider，后续继续迁入统一运行时。如果候选提示词复制了语料或答案片段，系统会先调用“去语料化修复”把具体词改成抽象规则，修复后仍泄露才拒绝。UI 会展示真实模型、实际并发、调用数、token、耗时和修复次数。
...
`v4.5.6` 起，主流程命名改为更贴近用户动作的 `项目总览 -> 定义与规范 -> 批量标注 -> 审核与修正 -> 结果与导出`。“定义与规范”页面顶部新增任务摘要，明确这里要选择项目、确认概念定义/边界/负例规则、维护 15 条金样例，并运行提示词验证或定义优化。
...
`v4.5.15` 起，Rosetta 增加本机 **Debug 调试追踪页**。使用 `--debug` 或 `ROSETTA_DEBUG_MODE=1` 启动后，可访问 `http://localhost:8501/debug`。页面会按子对话窗完整展示每次与 LLM 沟通的 system prompt、user prompt、assistant response、provider、model、temperature 和耗时。该模式会保存完整提示词、语料片段和模型回复，只建议本机排障时开启。

`v4.5.17` 起，debug 模式下直接打开 `http://localhost:8501/debug` 不再被强制 debug notice 弹窗或 `st.stop()` 拦截，可以直接进入调试追踪页；其他普通页面仍保留原有 debug notice 确认流程。

`v4.5.18` 起，通用标注 prompt 在 `概念定义` 和冻结 `标注格式` 之间固定预留 `相似参考样例` 槽位。这个槽位只在 top-k 验证或批量标注显式传入检索样例时填充；普通 `examples` 仍只用于推断标签，不会自动把 gold 样例或历史样例作为 few-shot 答案注入 prompt。检索本身仍使用本地 `rosetta-local-hash-384`，零 API、零 token；被注入的大模型标注 prompt 会产生正常 token。

`v4.5.16` 起，Debug 调试追踪页不再显示在主导航中，只保留直达地址 `http://localhost:8501/debug`。页面改成实时日志流，默认读取最新 debug session，每 `2` 秒自动刷新，并优先显示 `llm_chat` 事件；需要时仍可切换历史日志、筛选事件类型或搜索 prompt / response。
...
`v4.5.9` 起，概念验证、候选回测、单条标注和批量标注共用同一个运行时 prompt 框架；`v4.5.18` 后完整顺序为“概念定义 -> 相似参考样例 -> 标注格式 -> 通用格式示例 -> 待标注文本 -> 任务强调”。格式示例只解释 JSON 和 `[span]{Term}` / AnnotationDoc 的返回结构，不再混入当前任务 gold 或失败样例；批量上下文中的相似样例和失败记忆只作为概念边界参考，不承担输出协议说明。
...
| 提示词构成 | [docs/user/PROMPT_COMPOSITION.md](./docs/user/PROMPT_COMPOSITION.md) | 标注运行时 prompt 的中英文模板、冻结输出协议和维护同步规则 |
...
| 文档站语言切换 | MkDocs 顶部提供 `中文 / English` 语言切换入口；`English` 不出现在中文导航栏，英文页面使用英文导航，切换语言时优先保留当前页面 |
| 提示词构成对照 | 文档站新增中英文 prompt 构成页；程序侧运行时标注 prompt builder 提供 `zh-CN / en-US` 两套同构模板，主流程默认中文 |
| 内置案例 | 程序启动即内置“专业命名实体标注”官方样例，包含 15 条金样例和基础提示词 |
| 运行中防重复提交 | 长耗时按钮点击后会切换为运行中状态并禁用，避免重复触发验证、批量提交和审核保存 |
| 调试追踪页 | debug 模式下开放 `http://localhost:8501/debug`，不显示在主导航中，直达时不弹强制 notice；页面像日志流一样自动刷新大模型请求和回复，日志落盘到 `.runtime/logs/debug/` |
...
| 大模型服务运行时 | 提示词验证和定义优化使用统一运行时记录进度、token、耗时和重试；批量标注仍在逐步迁入同一运行时 |
...
| 统一标注 prompt 框架 | 概念验证、批量标注和候选回测统一使用“概念定义 -> 相似参考样例 -> 标注格式 -> 通用格式示例 -> 待标注文本 -> 任务强调”，避免输出协议混进可优化概念；相似参考样例只有显式检索后才填充 |
...
然后直接打开 `http://localhost:8501/debug` 查看完整 LLM prompt / response。该页面不会显示在主导航中，也不会触发普通页面的强制 debug notice；页面会每 2 秒自动刷新，并保留敏感调试日志，请勿在共享环境长期开启。
...
| 调试追踪 | 仅 debug 模式可通过 `/debug` 直达；不显示在主导航中，用实时日志流查看 LLM prompt / response 子对话和 debug 事件 |
