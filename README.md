...
Rosetta 是一个 **本地优先的 agentic annotation tool**。它面向需要快速建立标注任务的研究者、语言学家、数字人文团队和领域专家，把“一句话概念描述 + 15 条金样例”转成可运行、可复核、可导出的标注流水线。
...
| Debug 调试追踪页 | debug 模式下开放 `http://localhost:8501/debug`，不显示在主导航中，直达时不弹强制 notice；页面像日志流一样自动刷新 LLM prompt / response，日志落盘到 `.runtime/logs/debug/` |
