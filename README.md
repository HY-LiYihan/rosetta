...
`v4.5.1` 起，三方法提示词优化实验使用更严格的停止口径：每个方法独立训练，达到 `15/15` 立即成功停止；否则只有连续 `5` 轮 loss 没有下降才停止，默认最多运行 `30` 轮。报告中的最佳方法、最佳 loss 和最佳提示词使用历史最优接受版本，而不是最后一轮快照。统一 CLI 可运行内置“硬科学科普术语标注”真实 DeepSeek 对比实验，并输出 `comparison_report.md`、`comparison_result.json` 和 `prompt_evolution.jsonl`。
