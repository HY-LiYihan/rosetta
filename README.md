...
`v4.5.8` 起，“定义与规范”的临时概念表单进一步收敛为更少的必填项：用户只填写概念名称、概念描述或定义、边界说明，并从选项中选择冻结的标注输出协议。标签集合不再作为主输入项，而是从金样例 span label 自动推断，缺省为 `Term`；普通 span 任务使用 `JSON + [span]{Term}`，relation / attributes / 多层任务可选择完整 `AnnotationDoc` JSON 协议。
