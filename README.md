...
`v4.4.0` 起，概念实验室新增 **提示词优化训练**：系统可以在同一批 15 条金样例上公平比较 `只要求优化`、`失败反思` 和 `文本梯度 AdamW` 三种方法。每种方法独立运行，每轮生成候选提示词、回到金样例上计算 loss，只接受损失下降的干净版本；最终只把胜出的可用提示词写入 `ConceptVersion.description`，方法对比、loss 曲线、候选日志和原始响应进入 metadata / artifact。
...
`v4.5.0` 起，提示词优化训练接入统一 **LLM service runtime**：默认真实模型路径使用 DeepSeek `deepseek-v4-pro`，provider 级共享 semaphore 将全局默认并发上限提升到 `20`。15 条金样例验证、候选回测和批量任务会受同一 provider profile 约束；如果候选提示词复制了语料或答案片段，系统会先调用“去语料化修复”把具体词改成抽象规则，修复后仍泄露才拒绝。UI 会展示真实模型、实际并发、调用数、token、耗时和修复次数。
...
`v4.5.2` 起，概念实验室的提示词优化训练不再是同步 spinner 黑箱。点击后会创建后台 `WorkflowRun`，训练线程把 `RunProgressEvent` 写入 SQLite，页面每 2 秒轮询并展示当前阶段、ETA、模型调用、token、修复次数、最佳方法和实时日志；最终报告额外输出 `run_events.jsonl`，用于复现实验过程。
...
`v4.5.4` 起，“验证概念”不再是顺序黑箱等待：真实 LLM 模式会并发验证 15 条 gold，默认并发上限 `20`，页面显示进度条、运行中数量、已用时、ETA、调用数、token 和模型耗时。
...
| 提示词优化训练 | 在同一批 15 条金样例上比较“只要求优化”“失败反思”“文本梯度 AdamW”，达到 15/15 即成功，否则按连续无下降轮数或最大轮数停止 |
| LLM 服务运行时 | DeepSeek 默认模型为 `deepseek-v4-pro`，provider 级共享并发上限默认 `20`，记录调用进度、token、耗时和重试 |
...
| 冻结输出协议 | prompt optimizer 只优化概念语义；JSON schema、标签、markup、parser 和 format repair 由 Rosetta harness 固定注入 |
| 防背答案与去语料化修复 | 训练反馈允许看批改对照；候选提示词若复制语料或答案片段，会先修复为抽象规则，修复失败才拒绝 |
| 三方法真实对比实验 | `llm_optimize_only / llm_reflection / text_gradient_adamw` 从同一简单提示词出发，连续 5 轮 loss 无下降才停止，并输出 Markdown/JSON/JSONL 结果 |
...
第一次使用可以直接打开“概念实验室”。程序启动时会自动把主运行库刷新为官方样例“专业命名实体标注”，页面中已经有项目、概念阐释、15 条金样例和初始概念版本。
