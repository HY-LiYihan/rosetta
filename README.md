...
Rosetta 是一个 **本地优先的智能体式标注工具**。它面向需要快速建立标注任务的研究者、语言学家、数字人文团队和领域专家，把“一句话概念描述 + 15 条金样例”转成可运行、可复核、可导出的标注流水线。
...
Rosetta 也是一套可验证的研究工程：它要检验大模型智能体在低资源、概念可描述、任务边界会变化或任务不够常规的场景中，能否比传统 PLM-first 标注流程更快形成可审计的数据生产闭环。它不声称在完整高质量训练集条件下无条件超过 PLM，而是强调样本效率、概念迭代速度、人类审核收益和过程可追溯性。
