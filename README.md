...
`v4.5.15` 起，Rosetta 增加本机 **Debug 调试追踪页**。使用 `--debug` 或 `ROSETTA_DEBUG_MODE=1` 启动后，侧边栏会出现 `调试追踪`，也可直接访问 `http://localhost:8501/debug`。页面会按子对话窗完整展示每次与 LLM 沟通的 system prompt、user prompt、assistant response、provider、model、temperature 和耗时。该模式会保存完整提示词、语料片段和模型回复，只建议本机排障时开启。
...
| Debug 调试追踪页 | debug 模式下开放 `http://localhost:8501/debug`，按子对话窗完整查看 LLM prompt / response，日志落盘到 `.runtime/logs/debug/` |
...
然后打开 `http://localhost:8501/debug` 查看完整 LLM prompt / response。该页面会保留敏感调试日志，请勿在共享环境长期开启。
...
| 调试追踪 | 仅 debug 模式显示；查看完整 LLM prompt / response 子对话和 debug 事件 |
