...
`v4.5.8` 起，“定义与规范”的临时概念表单进一步收敛为更少的必填项：用户只填写概念名称、概念描述或定义、边界说明，并从选项中选择冻结的标注输出协议。标签集合不再作为主输入项，而是从金样例 span label 自动推断，缺省为 `Term`；普通 span 任务允许模型运行时返回简单 `JSON + [span]{Term}`，Rosetta 会解析为统一 AnnotationDoc / Prodigy-compatible 存储结构；relation / attributes / 多层任务可选择完整 `AnnotationDoc` JSON 协议。
...
`v4.5.11` 起，“定义与规范”页面主操作收敛为两个 tab：**提示词验证** 和 **提示词优化**。验证包含格式验证、无样例标注验证、带 top-k 相似 gold 参考的标注验证；优化包含人工优化、无样例自监督优化和第一版类训练优化。类训练优化每轮默认生成 5 个候选，逐个回测 15 条 gold，先选择本轮 loss 最低候选，再仅在它超过阈值下降时接受；系统会把 `旧 prompt -> 新 prompt -> loss 变化` 历史摘要反馈给下一轮，并在页面和 runtime 保存 v0 到 vn 的提示词版本。
