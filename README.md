...
| Debug 调试追踪页 | debug 模式下开放 `http://localhost:8501/debug`，不显示在主导航中；页面像日志流一样自动刷新 LLM prompt / response，日志落盘到 `.runtime/logs/debug/` |
...
然后直接打开 `http://localhost:8501/debug` 查看完整 LLM prompt / response。该页面不会显示在主导航中，会每 2 秒自动刷新，并保留敏感调试日志，请勿在共享环境长期开启。
